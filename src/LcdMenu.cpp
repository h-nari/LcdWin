...
LcdMenu::LcdMenu() : LcdWin()
{
  ...
  m_textColor = Humblesoft_ILI9341::rgb("black");
  m_x = 5;
  m_y = 5;
  m_w = 0;
  m_rowHeight = 0;
...
void LcdMenu::adjustSize()
{
  ...
  Serial.printf("%s:%d textSize:%u\n",__FUNCTION__,__LINE__,m_textSize);
...
bool LcdMenu::drawChanged(uint16_t cx, uint16_t cy)
{
...
bool LcdMenu::tp_pressed(int16_t x, int16_t y)
{
  m_selectedRow = -1;
  drawChanged(x,y);
  return true;
...
bool LcdMenu::tp_drag(int16_t x, int16_t y)
{
  drawChanged(x,y);
  return true;
}

bool LcdMenu::tp_released(int16_t x, int16_t y)
{
